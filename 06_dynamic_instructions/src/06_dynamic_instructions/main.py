...
async def main():
    ...
    print("\n🎭 Example 1: Basic Dynamic Instructions")
    print("-" * 40)
    result = await Runner.run(agent_basic, "Hello!", run_config=run_config)
    ...
    result1 = await Runner.run(agent_context, "Hello!", run_config=run_config)
    print("First message:")
    print(result1.final_output)
    
    result2 = await Runner.run(agent_context, "Tell me about Python", run_config=run_config)
    print("\nSecond message:")
    ...
    result = await Runner.run(agent_time, "How are you today?", run_config=run_config)
    ...
    for i in range(3):
        result = await Runner.run(agent_stateful, f"Question {i+1}: Tell me about AI",run_config=run_config)
    ...
    result = await Runner.run(agent_explorer, "What can you tell me about yourself?",run_config=run_config)
